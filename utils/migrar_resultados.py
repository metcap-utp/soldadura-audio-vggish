...
def migrate_json_file(json_path, project_type):
    ...
    for entry in data:
        try:
            new_entry = migrate_vggish_result(entry)
...
def process_duration_dir(duration_dir, create_backup_only=False):
    ...
    # Migrar
    migrated = migrate_json_file(json_path, "vggish")
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Migración de JSON - Proyecto: vggish-backbone")
    print(f"{'='*60}")
